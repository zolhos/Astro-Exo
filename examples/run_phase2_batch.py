...
SAMPLE_TARGETS = [
    {
        "tic_id": 231663901,
        "name": "TOI-101.01 (WASP-46b)",
        "toi": "101.01",
        "period_days": 1.43037,
        "t0_btjd": 1326.0091,
        "duration_hours": 1.62,
        "catalog_depth_ppm": 18960.7,
        "expected_disp": "KP"
    },
    {
        "tic_id": 149603524,
        "name": "TOI-102.01 (WASP-62b)",
        "toi": "102.01",
        "period_days": 4.411937,
        "t0_btjd": 3982.0655,
        "duration_hours": 3.63,
        "catalog_depth_ppm": 14711.9,
        "expected_disp": "KP"
    },
    {
        "tic_id": 336732616,
        "name": "TOI-103.01 (HATS-3b)",
        "toi": "103.01",
...
def adjust_epoch_to_dataset(t0_ref: float, period: float, time_arr: np.ndarray) -> float:
    """Adjusts reference transit epoch t0 by integer orbits to fall within the observed sector."""
...
def process_sample(target_info: Dict[str, Any], output_dir: str) -> Dict[str, Any]:
    ...
    print(f"\n=======================================================")
    print(f"Processando Alvo: {name} (TIC {tic})")
    print(f"Período: {period:.4f} d | Duração: {duration_h:.2f} h")
    print(f"=======================================================")

    t_start = time.perf_counter()

    # 1. Download / Obtenção de Curva de Luz e TPF
...
def main():
    output_dir = "results/phase2_samples"
    ...
    # 1. Exporta batch_summary.json e batch_summary.csv
    import csv
    ...
    print(f"\nResumo do lote salvo em:")
    print(f"  JSON: {summary_json}")
    print(f"  CSV:  {summary_csv}")

    # 2. Gera Figura Comparativa Científica dos 4 Alvos
    print("\nGerando painel comparativo dos 4 alvos...")
    ...
    for idx, art in enumerate(processed_artifacts):
        ...
        # Coluna 1: Curva de Luz Bruta + Tendência wotan
        ax1 = axes[idx, 0]
        ...
        # Coluna 2: Trânsito Dobrado em Fase
        ax2 = axes[idx, 1]
        ...
        # Coluna 3: Imagem de Diferença e Vetting WCS
        ax3 = axes[idx, 2]
    ...
    fig_path = os.path.join(output_dir, "phase2_4_samples_diagnostics.png")
